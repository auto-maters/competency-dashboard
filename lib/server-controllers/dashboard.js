    ...
    return new Promise(function (resolve) {
      try {
        db.task('get-everything', function (t) {
          return t.batch([t.any('SELECT emp_comp_level, COUNT(emp_comp_name) AS comp_level_count, emp_comp_name FROM emp_comp_updated GROUP BY emp_comp_level, emp_comp_name ORDER BY emp_comp_name'), t.any('SELECT emp_comp_name, COUNT(emp_id) AS emp_count FROM emp_comp_training WHERE emp_status=$1 GROUP BY emp_comp_name ORDER BY emp_comp_name', ['Yes'])]);
        }).then(function (data) {
